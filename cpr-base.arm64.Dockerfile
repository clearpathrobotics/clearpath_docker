...
# Index public images via the Clearpath rosdep generator.
# Note: the Clearpath stable apt repo only publishes amd64 packages, so it is
# intentionally NOT configured here. This image is for arm64 builds and relies
# solely on upstream Ubuntu/ROS package sources plus the rosdep metadata below.
